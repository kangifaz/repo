...
ggplot(mtdl_data, aes(x = Time, y = ROA, color = Covid_Period)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  geom_text(aes(label = round(ROA, 2)), vjust = -1, size = 3.5, color = "black") + # Menambahkan label
  ...
  ggtitle("Grafik ROA dengan Penandaan Periode Covid-19 dan Label") +
  xlab("Waktu") + ylab("ROA") +
  theme_minimal() +
