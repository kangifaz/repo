...
ggplot(mtdl_data, aes(x = Time, y = ROE, color = Covid_Period)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  geom_text(aes(label = round(ROE, 2)), vjust = -1, size = 3.5, color = "black") +
  ...
  ggtitle("Grafik ROE dengan Penandaan Periode Covid-19 dan Label") +
  xlab("Waktu") + ylab("ROE") +
  theme_minimal() +
