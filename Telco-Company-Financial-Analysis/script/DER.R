...
ggplot(mtdl_data, aes(x = Time, y = DER, color = Covid_Period)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  geom_text(aes(label = round(DER, 2)), vjust = -1, size = 3.5, color = "black") +
  ...
  ggtitle("Grafik DER dengan Penandaan Periode Covid-19 dan Label") +
  xlab("Waktu") + ylab("DER") +
  theme_minimal() +
